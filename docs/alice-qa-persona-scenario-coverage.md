# Alice QA persona-to-scenario coverage

Alice QA persona-to-scenario coverage connects editable instructor and student
...
3. Validate the full persona and scenario inventory:

   ```bash
   cargo run -q -p eatme-cli -- assets validate --json
   ```

4. Confirm the generated Gadugi adapter is fresh:
...
| `purpose` | Describes the editable workshop facilitation evidence. |
...
| `agentic_flow.expected_outputs` | Instructor-facing artifacts such as a workshop plan, checkpoint board, helper roles, recovery moves, and showcase notes. |
| `agentic_test_prompt` | Prompt for producing a two-hour Alice workshop facilitation plan. |
| `acceptance_criteria` | Given/when/then checks for timeboxing, mixed readiness, participant progress, and reviewable evidence. |
...
| `timeouts.agentic_seconds` | Agentic review timeout. |
| `artifacts` | Logical `agentic://` output names for the instructor-facing artifacts. |
...
This feature proves a small, explicit coverage connection between the Alice crew
persona inventory and an editable scenario asset. It is not exhaustive for every
Alice persona or every classroom situation.
