...
```bash
export ALICE_HOME=$HOME/src/alice3-modernization
```
...
The committed gadugi adapter uses `EATME_REPO` when provided and otherwise runs from the current working tree, so it does not depend on one checkout path.
