...
| Setup and first use | `instructor-classroom-setup-readiness` | Instructor prepares lab machines, confirms Alice can create a starter world, and saves setup evidence. | Covered: editable scenario validates and runnable instructor agentic checklist/fallback/student-note output evidence is emitted without claiming desktop launch success | Covered: web setup/readiness API evidence is covered; desktop setup checks are not claimed |
