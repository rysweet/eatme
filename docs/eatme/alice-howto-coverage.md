...
| Classes | `modified-class-portability` | Student saves a modified class, imports it into another project, and checks that behavior travels with it. | Covered | Covered: browser UI class behavior export/import/save/reopen evidence is covered via LookingGlass #266; desktop Alice portability is bounded by EatMe's runnable tools/eatme-class-portability contract |
