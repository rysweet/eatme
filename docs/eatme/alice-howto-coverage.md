...
| VR | `vr-camera-locomotion-journey` | Student records VR camera comfort planning and checks bounded movement-comfort evidence. | Covered | Covered: bounded browser WebXR locomotion API evidence and camera comfort evidence are covered; true headset/native VR remains explicitly unsupported |
