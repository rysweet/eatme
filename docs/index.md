...
| Auditing instructor/student lesson-session readiness and RabbitHole no-go evidence | [Lesson Session Readiness](lesson-session-readiness.md) |
...
| `first-lessons-real-ui-actions` | Students and reviewers | Scenario-labeled launch plus Alice window detection, RabbitHole Run-window/desktop execution evidence when available, `ui-action-contract.json` expectations, and explicit no-go behavior when evidence is missing or insufficient. |
