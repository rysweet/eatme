...
| Scenario | What the user is trying to do | Silver-thread gap still open | Evidence still needed |
| --- | --- | --- | --- |
| First lesson real UI actions | Start Alice, see the lesson window, and try the first visible classroom actions with clear stopping points. | Can a learner move from opening Alice to one visible action while knowing exactly where the current evidence stops? | A learner-facing run showing the visible action attempted, what appeared on screen, where the learner had to stop, and the prompt they saw next. |
| Starter project open, save, and export preflight | Open a starter project, choose one small change to try, and understand what still needs to be checked before relying on the project in class. | Can the learner return to the project later or share an exported artifact without help? | A reproducible learner journey showing the opened project, the change attempt, the later return or export attempt, and the exact result shown to the learner. |
| Setup preflight ready to create | Check that a classroom machine is ready before students start creating in Alice. | Can the class move from setup checks into a first creation task without losing time to missing tools, unclear accounts, or unavailable starter files? | Instructor setup notes showing the machine state, student access path, starter-file availability, and the first creation prompt students received. |
| Instructor lesson materials remix | Prepare a teacher plan, student handout, and exit ticket from Alice lesson resources. | Can students use the remixed packet in class without the instructor filling in hidden steps? | Instructor-facing classroom notes showing which prompts students used, what confused them, and what the instructor changed before reuse. |
| Instructor-to-student launch handoff | Turn launch and window evidence into a plain student prompt for recording one Alice action, visible result, and next revision. | Can students tell the difference between "Alice opened on this machine" and "my project behavior worked"? | Student-visible handoff samples and instructor notes showing what students recorded, where they needed help, and which evidence stayed environment-only. |
| Student outcomes discussion rubric | Discuss student Alice projects using visible behavior, learner explanation, process evidence, and one next revision. | Can different instructors use the discussion frame consistently without treating it as automatic grading or creative scoring? | Instructor-visible rubric use notes showing examples of visible behavior, student explanation, process evidence, and revision feedback across varied projects. |
| Classroom gallery walk and rubric | Help students review one another's Alice projects with respectful evidence, questions, and revision ideas. | Can a live group give useful peer feedback while protecting student privacy and keeping revision expectations clear? | Classroom-facing gallery notes showing peer observations, creator responses, revision checkpoints, and instructor boundary notes without private student records. |
| Workshop facilitator live studio | Run a short Alice studio where students build, observe, revise, and share one small behavior. | Can the room progress through setup, build, run, revise, and share checkpoints with student-owned evidence at each stop? | A facilitator packet and classroom notes showing checkpoint timing, help signals, one visible behavior per pair, revision notes, and share-out evidence. |
