...
| Modernized Run-window evidence | `run_world_desktop_toolbar_window_observed` assertion on the `modernized` launch manifest | No RabbitHole evidence that the Run animation started after the toolbar dispatch. Detection uses the sentinel file first; if absent, a pre/post screenshot comparison detects scene panel changes (see [Issue #252](issue-252-procedure-selector-and-run-observation.md)). The older `run_world_desktop_window_observed` shortcut assertion may appear in action evidence, but it is not the modernized RabbitHole readiness check. |
...
| Screenshot artifact | `screenshots/run-window-after-dispatch.png` next to the modernized `ui-action-contract.json`, canonicalized under the comparison evidence root. Additionally, `screenshots/scene-before-run-click.png` and `screenshots/scene-after-run-click.png` are produced for the toolbar-path screenshot comparison (see [Issue #252](issue-252-procedure-selector-and-run-observation.md)). | Missing file, empty file, unreadable file, symlink escape, or artifact outside the expected evidence root. |
