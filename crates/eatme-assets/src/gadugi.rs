...
/// Shared preflight step block template (use: alice-preflight).
/// Placeholders: `{{run-id}}`, `{{expected-scenario-asset-count}}`.
const PREFLIGHT_STEP_BLOCK: &str =
...
fn has_preflight_steps(scenario: &EatmeScenarioAsset) -> bool {
    scenario.steps.len() >= 2
        && scenario.steps[0].id == "validate-assets"
        && !scenario.steps[0].command.contains("--path")
        && scenario.steps[1].id == "check-dependencies"
}

fn preflight_steps_from_template(
    run_id: &str,
    expected_scenario_asset_count: usize,
