...
pub(super) fn desktop_next_action_summary(
    ...
    Some(DesktopNextActionSummary {
        ...
        observations,
        does_not_prove: claims::desktop_next_action_non_claims(&evidence.does_not_claim),
    })
}
...
fn boundary_does_not_prove(boundary: &FirstLessonEvidenceBoundary) -> Vec<String> {
    ...
    if boundary.id == "save_project" {
        push_unique(&mut claims, claims::save_completion_non_claim());
        push_unique(&mut claims, claims::first_lesson_completion_non_claim());
    }
