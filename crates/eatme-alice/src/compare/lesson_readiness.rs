                ...
                Ok(text) => match serde_json::from_str::<serde_json::Value>(&text) {
                    Ok(contract) => {
                        ...
                        if role == "modernized" {
                            ...
                            match evidence_root.canonicalize() {
                                Ok(evidence_root) => {
                                    let pixel_boundary =
                                ...
                                Err(error) => issues.push(format!(
                                    "{role} comparison evidence root could not be resolved at {}: {error}",
